    ...
    if (elements.roastForm) { 
        ...
        elements.roastForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Stop enhanced loading with failure status
                if (typeof stopEnhancedLoading === 'function') {
                    try {
                        stopEnhancedLoading(false);
                    } catch (e) {
                        console.warn('Error stopping enhanced loading:', e);
                    }
                }
                ...
                if (error.message) {
                    if (error.message.toLowerCase().includes('network') || 
                        ...
                        errorMessage = 'Network error while generating your report. Please check your connection and try again.';
                ...
                // Signal successful loading completion if enhanced loading is enabled
                if (typeof stopEnhancedLoading === 'function') {
                    try {
                        console.log('Signaling successful loading completion...');
                        stopEnhancedLoading(true); // Signal success (jumps to 100%)
                    } catch (e) {
                        console.warn('Error signaling loading completion:', e);
