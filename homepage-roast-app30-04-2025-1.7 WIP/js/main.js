...
document.addEventListener('DOMContentLoaded', () => {
    console.log('DOMContentLoaded event fired.'); // Log that the event fired
    // --- Get DOM Elements --- 
    const elements = {
    ...
    console.log('Attempted to get roastForm element:', elements.roastForm); // Log the result

    let currentRoastData = { roastText: '', analyzedUrl: '' }; // Store current results for sharing

    // Provide elements to the UI module
    // Check if setDOMElements exists before calling
    if (typeof setDOMElements === 'function') {
        ...
        // Optionally return or handle this error appropriately
    }
    ...
    // Form Submission
    // Add a check to ensure roastForm was actually found
    if (elements.roastForm) { 
        ...
        elements.roastForm.addEventListener('submit', async (e) => {
            ...
            console.log(`URL submitted: ${url}`); // Add log
            // Example: Show loading (check if function exists)
            if (typeof showLoading === 'function') {
                 showLoading(); // Call the function from ui.js
            } else {
        ...
        console.error('ERROR: roastForm element was null. Could not attach submit listener.');
    }
